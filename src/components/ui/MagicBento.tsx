...
const BentoCardGrid: React.FC<{
    ...
    <div
        className="bento-section grid gap-4 p-4 max-w-7xl mx-auto select-none relative"
            ...
            <BentoCardGrid gridRef={gridRef}>
                <div className="card-responsive">
                    {cardsToRender.map((card, index) => {
                        const baseClassName = `card flex flex-col justify-between relative min-h-[220px] w-full p-6 rounded-[24px] border border-white/10 bg-neutral-900 overflow-hidden transition-all duration-300 ease-in-out hover:-translate-y-1 hover:shadow-xl ${enableBorderGlow ? 'card--border-glow' : ''
                            }`;
                        ...
                        const content = (
                            <>
                                ...
                                <div className="card__header flex justify-between gap-3 relative text-white z-10 w-full mb-auto">
                                    {card.label && (
                                        <span className="card__label px-3 py-1 rounded-full text-xs font-medium bg-white/10 backdrop-blur-md border border-white/10">{card.label}</span>
                                    )}
                                </div>

                                <div className="card__content flex flex-col relative text-white z-10 mt-4">
                                    <h3 className={`card__title font-bold text-2xl mb-2 tracking-tight ${textAutoHide ? 'group-hover:text-clamp-none' : ''}`}>
                                        {card.title}
                                    </h3>
                                    <p
                                        className={`card__description text-sm text-neutral-300 leading-relaxed ${textAutoHide ? 'text-clamp-2' : ''}`}
                                    >
                                        {card.description}
                                    </p>
                                </div>
